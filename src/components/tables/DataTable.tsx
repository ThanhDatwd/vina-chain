...
import { PaginationQuery, Integration } from "@/models/commonn";
import { Dropdown } from "../Dropdown";
import { numberWithComma, nFormatter } from "@/utils/formatNumber";
...
const defaultData = [
  {
    id: "",
    txnHash: `${vinachainAddress?.slice(0, 9)}...`,
    method: "transfer",
    block: "Block",
    age: "12 second ago",
  ...
  return (
    <div className="flex flex-wrap  rounded-3xl">
      <div className="w-full max-w-full flex-0 ">
        <div className="relative flex flex-col min-w-0 break-words bg-white dark:bg-[#111111] rounded-2xl  border border-[#bdc5d133] bg-clip-border">
          <div className="fixed-columns">
            {defaultHeaderTable ? (
              <div className="flex flex-col md:flex-row gap-3 md:gap-0 md:justify-between p-4 md:p-5 pb-8">
                <div> {childrenHeaderTable && childrenHeaderTable}</div>
                <div className=" flex flex-col md:flex-row  md:items-center justify-end gap-2">
                  {isDownloadData && (
                    <button className="h-[28.8px] w-fit flex items-center justify-center gap-2 text-[12.5px] text-dark900 dark:text-gray200 border border-[#bdc5d133] hover:bg-[#eaecef] font-light dark:hover:bg-[#222] py-1 px-2 rounded-md">
                      <DownloadIcon />
                      Download Page Data
                    </button>
                  )}
            ...
            <div className="overflow-x-auto">
              <div className={classNames(`scrollbar ${theme === THEME.DARK ? "dark" : ""}  overflow-auto pb-2`)}>
            ...
            {showPagination ? (
              <div className="px-4">
                {data.length === 0 && (
                  <div className="top-1/2 left-1/2 right-1/2 text-[32px] my-10 flex justify-center">
