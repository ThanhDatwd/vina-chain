    ...
    <ScanLayout containerStyle="bg-[#FAFAFA] dark:bg-primaryDark">
      <div className="px-3 lg:px-[283px] pt-12 pb-16">
        <div className="flex justify-center w-full">
            ...
            <h2 className="text-[30px] text-dark900 font-bold">{t("vinaScan.brandAssets.title")}</h2>
            <span className="mb-4">{t("vinaScan.brandAssets.subTitle")}</span>
            </div>
        </div>
        <hr className="line" />
        <div className="content-heading">
          <h3 className="text-dark900 font-medium text-[22.5px]">
        ...
        <div>
          <div className="content-heading">
            ...
            <span className="mb-4">{t("vinaScan.brandAssets.brandingGuidelinesContent1")}</span>
          </div>
        ...
        <div className="content-heading">
          ...
          <span className="mb-10">{t("vinaScan.brandAssets.downloadsContent")}</span>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 mb-5">
          {Array.from({length: 4}, (_, index) => (
            <div key={index} className="mt-4 text-[15px] text-gray550 rounded-[11px] border border-[#e9ecef]">
              <div
                className="h-[250px]"
                style={{
              ...
              />
              <div className="w-full p-4 border-t border-[#e9ecef]">PNG & SVG</div>
            </div>
          ))}
        </div>
        <div className="flex justify-center">
          <button className="flex items-center gap-1 button-primary text-[15px]">
            <CopyIcon color="#fff"/>
            <label className="capitalize">{t("vinaScan.brandAssets.downloadButton")}</label>
          </button>
        </div>
        <hr className="line"/>
        <div className="content-heading">
          <h3 className="text-dark900 font-medium text-[22.5px]">{t("vinaScan.brandAssets.agreement")}</h3>
          <div>
            ...
            <Link href={"/scan/contactUs"}>{t("vinaScan.contactUs.title")}</Link>
          </div>
        </div>
      </div>
    </ScanLayout>
